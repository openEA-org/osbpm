...
function putHisData(htmlCode){
	if(htmlCode==undefined){
		if(hisCurrentCode==hisEndCode){return;} //说明没有变化的情况下不用存历史记录
		hisCurrentIndex++;
...
function undo(){
	hisCurrentIndex--;
	if(hisCurrentIndex<0){hisCurrentIndex=0;}
...
function redo(){
	...
	if(htmlCode=="" || htmlCode==undefined){return;}
	if(flowframe.document.all.div1.innerHTML!=htmlCode){
		flowframe.document.all.div1.innerHTML=htmlCode;
		fixNode();
	}
...
var TreeLoader=new Ext.tree.TreeLoader({
			dataUrl:'rule?wf_num=R_S030_B005',
			requestMethod:'POST'
		});
...
Ext.extend(westPanel, Ext.tree.TreePanel, {
   ...
   getnode:function(id){this.getNodeById(id)}
});
...
var mainPanel= new Ext.Panel({
        ...
        region:'center',
        margins:'0 0 0 0',
        html:"<iframe name='flowframe' id='flowframe' src='rule?wf_num=R_S030_B002&Processid="+top.processid+"' width=100% height=100% ></iframe>"
    });
    ...
    west.on('contextmenu',function(node,event)
  {
    ...
    var menu = new Ext.menu.Menu({
          ...
          items: [
            new Ext.menu.Item({
                text: '删除属性',
                handler:function(){DeleteNode(nodeid,node.text);}
            })
         ]
         });
    ...
    id: 'MulSelMenu',
    items: [
      {text:'垂直自动对齐',id:'XDC',icon:'linkey/bpm/images/icons/page-next.gif',cls: 'x-btn-text-icon bmenu',handler:function(){flowframe.MulSel_GropToSameX()}},
      {text:'水平自动对齐',id:'YDC',icon:'linkey/bpm/images/icons/page-next.gif',cls: 'x-btn-text-icon bmenu',handler:function(){flowframe.MulSel_GropToSameY()}},
      {text:'垂直自动间距',id:'YJZ',icon:'linkey/bpm/images/icons/page-next.gif',cls: 'x-btn-text-icon bmenu',handler:function(){flowframe.MulSel_AutoYJZ()}},
      {text:'水平自动间距',id:'XJZ',icon:'linkey/bpm/images/icons/page-next.gif',cls: 'x-btn-text-icon bmenu',handler:function(){flowframe.MulSel_AutoXJZ(false)}}
    ]
    });
	...
	text:'保存流程',
	handler:function(){flowframe.save();}
	},'-',{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/images/icons/save.gif',
	text:'保存为图片',
	handler:function(){location.href="r?wf_num=R_S030_B008&Processid="+processid;}
	},'-',{
	cls: 'x-btn-text-icon bmenu',
	...
	},'-',
	{
		...
		text:'显示网格',
		handler:function(){flowframe.ShowGrid();}
	},'-',
	{
		cls: 'x-btn-text-icon bmenu',
		icon: 'linkey/bpm/images/icons/page-prev.gif',
		text:'左移(L)',
		handler:function(){flowframe.MoveCenter('left');}
	},'-',
	{
		cls: 'x-btn-text-icon bmenu',
		icon: 'linkey/bpm/images/icons/ns-collapse.gif',
		text:'上移',
		handler:function(){flowframe.MoveCenter('top');}
	},'-',
	{
		cls: 'x-btn-text-icon bmenu',
		icon: 'linkey/bpm/images/icons/ns-expand.gif',
		text:'下移',
		handler:function(){flowframe.MoveCenter('bottom');}
	},'-',
	{
		cls: 'x-btn-text-icon bmenu',
		icon: 'linkey/bpm/images/icons/page-next.gif',
		text:'右移',
		handler:function(){flowframe.MoveCenter('right');}
	},'-',
	{
		menu:MulSelXYMenu,
		cls: 'x-btn-text-icon bmenu',
		...
		text:'横向泳道',
		handler:function(){flowframe.mySetType('AddXNode');}
	},'-',{
		cls: 'x-btn-text-icon bmenu',
		icon: 'linkey/bpm/images/icons/class.gif',
		text:'纵向泳道',
		handler:function(){flowframe.mySetType('AddYNode');}
	},'-',
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/text.gif',
	text:'文字',
	handler:function(){flowframe.mySetType('AddText');}
	},'-',
	{
		cls: 'x-btn-text-icon bmenu',
		icon: 'linkey/bpm/images/icons/del.gif',
		text:'关闭窗口',
		handler:function(){ top.close();}
	}
	); //tb end
	...
	text:'移动',
	handler:function(){flowframe.mySetType('Move');}
	},{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/mouse.gif',
	text:'多选',
	handler:function(){flowframe.mySetType('MulSel');}
	},{
		cls: 'x-btn-text-icon bmenu',
		icon: 'linkey/bpm/ext/wf/images/editdx.gif',
		text:'修改大小',
		handler:function(){flowframe.mySetType('EditArea');}
	},'-',
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/line.gif',
	text:'直线',
	handler:function(){flowframe.mySetType('PolyLine','Line');}
	},
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/lineleftcur.gif',
	text:'左右折线',
	handler:function(){flowframe.mySetType('PolyLine','Bian');}
	},
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/linerightcur.gif',
	text:'上下折线',
	handler:function(){flowframe.mySetType('PolyLine','Bottom');}
	},
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/line90_1.gif',
	text:'上下直角',
	handler:function(){flowframe.mySetType('PolyLine','zhexian902');}
	},
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/line90_2.gif',
	text:'左右直角',
	handler:function(){flowframe.mySetType('PolyLine','zhexian90');}
	},
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/linecurv.gif',
	text:'曲线',
	handler:function(){flowframe.mySetType('PolyLine','zhexian');}
	},'-',
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/activity.gif',
	text:'人工活动',
	handler:function(){window.frames["flowframe"].mySetType('AddActivity');}
	},
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/subprocess.gif',
	text:'子流程',
	handler:function(){flowframe.mySetType('AddSubProcess');}
	},
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/autoactivity.gif',
	text:'自动活动',
	handler:function(){flowframe.mySetType('AddAutoActivity');}
	},
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/edge.gif',
	text:'网关',
	handler:function(){flowframe.mySetType('AddEdge');}
	},
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/start.gif',
	text:'开始',
	handler:function(){flowframe.mySetType('AddStart');}
	},
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/end.gif',
	text:'结束',
	handler:function(){flowframe.mySetType('AddEnd');}
	},
	{
	cls: 'x-btn-text-icon bmenu',
	icon: 'linkey/bpm/ext/wf/images/event.gif',
	text:'事件',
	handler:function(){flowframe.mySetType('AddEvent');}
	},'-'
	);
...
var url=location.href;if(url.indexOf("rule?wf_num=R_S030_B001")==-1){docuemnt.body.innerHTML="";}
function DeleteNode(Nodeid,text){
	if(!confirm("您确认要删除\""+text+"\" 吗?")){return false;}
