...
function serializeForm(){
	...
	formData="WF_TableName=BPM_GridList&"+formData;
	return formData;
}
...
function PreviewView(){
	window.open("../customgrid?wf_num=<%=gridNum%>");
}
...
function openJsonData(){
	...
	mask();
	$.get("../rule?wf_num=R_S001_B026&Dataid="+dataSource,function(data){
		var url="datafieldconfig.jsp?&wf_dtype=1&wf_eldocunid="+data+"&fromview=1";
		parent.addtab(url,"数据列配置","jsoncolumn","icon-config");
		var url="json_attribute.jsp?&wf_dtype=2&wf_eldocunid="+data+"&fromview=1";
		parent.addtab(url,"数据源属性","jsonattribute","icon-config");
		unmask();
	});
}
...
<tr><td valign="middle" width="21%" align="right">Html Header:</td>
<%
String htmlHeader=eldoc.g("HtmlHeader");
if(Tools.isBlank(htmlHeader)){
	htmlHeader="AppGridHtmlHeader";
}
%>
<td width="79%"><input value="<%=htmlHeader%>" name="HtmlHeader" id="HtmlHeader" class="easyui-validatebox" required size="60"/></td>
</tr>
