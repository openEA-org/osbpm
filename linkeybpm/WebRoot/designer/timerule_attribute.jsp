...
$(function(){
	if($("#RuleNum").val()==""){
		$("#RuleNum").val("R_"+$('#WF_Appid').val()+"_T");
	}
})
...
<td width="79%"><input value="<%=eldoc.g("ClassPath")%>" name="ClassPath" id="ClassPath" size='60' style="border:none" /></td>
</tr>

<tr><td valign="middle" width="21%" align="right">指定运行时间(表达式):</td>
<td width="79%"><input value="<%=eldoc.g("RuleSchedule")%>" name="RuleSchedule" id="RuleSchedule" size='52' class="easyui-combobox easyui-validatebox" data-options="valueField:'value',textField:'text',data: [{text: '半小时执行一次',value: '0 /30 * * * ?'},{text: '每小时执行一次',value: '0 0 /1 * * ?'},{text: '每月最后一个星期六执行',value: '0 0 22 ? * 7L'},{text: '每月1号执行',value: '0 0 0 1 * ? *'},{text: '每五分钟执行一次',value: '0 0/5 * * * ?'},{text: '每15秒执行一次',value: '0/15 * * * * ?'}]" required style="color:blue;font-size:16px" /></td>
</tr>

<tr><td valign="middle" width="21%" align="right">指定运行服务器:</td>
<td width="79%">
<select name="ServerIP" id="ServerIP"  >
<%
	String ServerIP=eldoc.g("ServerIP");
	sql="select ServerName,ServerIP from BPM_ClusterSeverList";
	Document[] dc=Rdb.getAllDocumentsBySql(sql);
	int sflag=0;
	for(Document doc:dc){
		if(doc.g("ServerIP").equals(ServerIP)){
			sflag=1;
			out.println("<option value=\""+doc.g("ServerIP")+"\" selected >"+doc.g("ServerName")+"</option>");
		}else{
			out.println("<option value=\""+doc.g("ServerIP")+"\" >"+doc.g("ServerName")+"</option>");
		}
	}
	if(sflag==0){
		out.println("<option value='*' selected >所有服务器</option>");
	}else{
		out.println("<option value='*' >所有服务器</option>");
	}
%>
</select>
</td>
