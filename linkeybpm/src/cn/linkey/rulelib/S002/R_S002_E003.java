 ...
 * @author admin
 * @version: 8.0
 * @Created: 2014-05-14 11:18
 */
final public class R_S002_E003 implements LinkeyRule {
    @Override
    ...
    public void formatRowData(Document doc) {
        ...
        String sql = "select FolderName from BPM_NavTreeNode where Treeid='T_S002_001' and Folderid='" + doc.g("Folderid") + "'";
        String folderName = Rdb.getValueBySql(sql);
        doc.s("Folderid", folderName);

        //获得表单名称
        sql = "select FormName from BPM_FormList where FormNumber='" + doc.g("FormNumber") + "'";
        doc.s("FormName", Rdb.getValueBySql(sql));
