 ...
 * @author admin
 * @version: 8.0
 * @Created: 2014-10-17 11:59
 */
final public class R_S016_E010 implements LinkeyRule {
    ...
    public String onViewOpen(Document viewDoc, String processid) throws Exception {
        ...
        String sql = "select distinct nodename,processid from BPM_InsUserList where processid='" + processid + "'";
        String processsql = "select nodename from bpm_modprocesslist where processid='" + processid + "'";
        //	    System.out.println("sql:"+processsql);

        String processname = Rdb.getDocumentBySql(processsql).g("nodename");
        Document[] document = Rdb.getAllDocumentsBySql(sql);
        for (Document doc : document) {
            countsql = "select nodename from BPM_InsUserList where processid='" + doc.g("processid") + "'and  nodename='" + doc.g("nodename") + "' and IsBackFlag<>0";
