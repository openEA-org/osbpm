...
/**
 * @RuleName:待办最长延时前10用户
 * @author admin
 * @version: 8.0
 * @Created: 2014-10-22 10:06
 */
final public class R_S016_E031 implements LinkeyRule {
    ...
    public String onViewOpen(Document viewDoc) throws Exception {
    	...
    	//String sql = "select Userid,CONVERT(int,overdatenum) as time from BPM_InsUserList where Status='current' and OverDateNum<>0 order by time desc";
    	//String DBType = Tools.getProperty("DBType");
    	//20190328 改为自动获取当前数据库类型
    	String DBType = Rdb.getDbType();
    	String sql = SqlType.createSql(DBType, "R_S016_E031_SQL1");
        ...
        if (Rdb.getCountBySql(sql) > 0) {
            for (Document doc : document) {
                ...
                if (Rdb.getDocumentBySql(namesql).g("cnname").length() > 0 && i < 10 && (nodename.indexOf(Rdb.getDocumentBySql(namesql).g("cnname")) == -1)) {
