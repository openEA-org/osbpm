 ...
 * @author admin
 * @version: 8.0
 * @Created: 2014-10-20 11:08
 */
final public class R_S016_E016 implements LinkeyRule {
    ...
    public String onViewOpen(Document viewDoc) throws Exception {
    	...
    	//String sql = "select top 10 WF_processid,cast(avg(numb) as numeric(10,4)) as tmp from (SELECT wf_processid,convert(int,WF_TotalTime) as numb FROM BPM_ArchivedData where wf_totaltime is not null) as f group by Wf_processid  order by tmp asc";
        //String DBType = Tools.getProperty("DBType");
    	//20190328 改为自动获取当前数据库类型
    	String DBType = Rdb.getDbType();
    	String sql = SqlType.createSql(DBType, "R_S016_E016_SQL1");
