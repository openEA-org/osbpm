 ...
 * @author admin
 * @version: 8.0
 * @Created: 2014-10-20 15:44
 */
final public class R_S016_E024 implements LinkeyRule {
    ...
    public String onViewOpen(Document viewDoc) throws Exception {
    	...
    	//String sql = "SELECT top 10 userid,count(*) as num,avg(datediff(minute,convert(datetime,starttime),convert(datetime,FirstReadTime))) as time FROM BPM_InsRemarkList where FirstReadTime<>'' group by userid order by time desc ";
        //String DBType = Tools.getProperty("DBType");
    	//20190328 改为自动获取当前数据库类型
    	String DBType = Rdb.getDbType();
    	String sql = SqlType.createSql(DBType, "R_S016_E024_SQL1");
