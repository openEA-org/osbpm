 ...
 * @author admin
 * @version: 8.0
 * @Created: 2014-10-16 17:48
 */
final public class R_S016_E004 implements LinkeyRule {
    ...
    public String onViewOpen(Document viewDoc) throws Exception {
        ...
        //图表JsHeader中用[X]字段名[/X]标识,viewDoc.s("字段名","10,20,30")来动态输出计算好的数据,
        String processsql = "SELECT DISTINCT WF_ProcessName FROM BPM_MainData";
        String countsql = "";
        String processname = "[";
        String count = "[";
        Document[] document = Rdb.getAllDocumentsBySql(processsql);
        for (Document doc : document) {
            countsql = "select WF_ProcessName from BPM_MainData where WF_ProcessName='" + doc.g("WF_ProcessName") + "'";
