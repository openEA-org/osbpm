...
/**
 * @RuleName:执行超时次数最多前10名
 * @author admin
 * @version: 8.0
 * @Created: 2014-10-20 14:06
 */
final public class R_S016_E021 implements LinkeyRule {
    ...
    public String onViewOpen(Document viewDoc) throws Exception {
    	...
    	//String sql = "select top 10 userid,COUNT(*)as count  from   BPM_InsRemarkList where OverTimeFlag<>0 group by userid order by count desc ";
        //String DBType = Tools.getProperty("DBType");
    	//20190328 改为自动获取当前数据库类型
    	String DBType = Rdb.getDbType();
    	String sql = SqlType.createSql(DBType, "R_S016_E021_SQL1");
