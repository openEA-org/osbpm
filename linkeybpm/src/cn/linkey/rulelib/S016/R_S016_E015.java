 ...
 * @author admin
 * @version: 8.0
 * @Created: 2014-10-20 10:53
 */
final public class R_S016_E015 implements LinkeyRule {
    ...
    public String onViewOpen(Document viewDoc) throws Exception {
        ...
        //String sql = "select top 10 userid,cast(sum(numb) as numeric(10,4))/(select COUNT(Userid) from BPM_InsUserList where Userid=f.userid and Status='end')as tmp from (SELECT userid,convert(int,totaltime) as numb FROM BPM_InsUserList as f where status='end' and Userid is not null) as f group by userid  order by tmp desc";
        //String DBType = Tools.getProperty("DBType");
    	//20190328 改为自动获取当前数据库类型
    	String DBType = Rdb.getDbType();
    	String sql = SqlType.createSql(DBType, "R_S016_E015_SQL1");
