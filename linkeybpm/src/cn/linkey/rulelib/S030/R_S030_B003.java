 ...
 */
final public class R_S030_B003 implements LinkeyRule {
    @Override
    public String run(HashMap<String, Object> params) {
        ...
        if (Rdb.getDbType().equals("MSSQL")) {
            sql = "select * from BPG_ModPluginConfig where (','+NodeType+',' like '%," + extNodeType + ",%' or NodeType='*') and Status='1' order by SortNum";
        }
        else if (Rdb.getDbType().equals("MYSQL")) {
            sql = "select * from BPG_ModPluginConfig where (concat(',',NodeType,',') like '%," + extNodeType + ",%' or NodeType='*') and Status='1' order by SortNum";
        }
        else {
            sql = "select * from BPG_ModPluginConfig where (','||NodeType||',' like '%," + extNodeType + ",%' or NodeType='*') and Status='1' order by SortNum";
        }
        ...
        //获得缺省html代码
        sql = "select DefaultCode from BPM_DevDefaultCode where  CodeType='BPG_ProcessNodeAtrrEditor'";
