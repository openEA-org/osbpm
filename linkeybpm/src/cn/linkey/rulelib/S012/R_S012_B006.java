...
/**
 * @RuleName:传送数据到oracle中去
 * @author admin
 * @version: 8.0
 * @Created: 2014-09-20 19:38
 */
final public class R_S012_B006 implements LinkeyRule {
    @Override
    public String run(HashMap<String, Object> params) throws Exception {
        ...
        try {
            int m = 0;
            conn = Rdb.getNewConnection("oracle");
            ...
            for (String tableName : tableSet) {
                ...
                for (Document doc : dc) {
                    // BeanCtx.out("准备存入doc="+doc);
                    // doc.setDbType("ORACLE"); //设置为oracle数据库类型
                    // doc.setTableFdConfig(Rdb.getTableColumnName(conn,tableName));
                    // BeanCtx.out("字段配置值为="+doc.getTableFdConfig());
                ...
                BeanCtx.p(tableName + "表共成功传送(" + i + ")条数据,(<font color=red>" + j + "</font>)条数据失败<br>");
            }
        }
        catch (Exception e) {
            BeanCtx.p("出错了<br>");
