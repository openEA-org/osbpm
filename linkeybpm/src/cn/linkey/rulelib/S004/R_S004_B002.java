 ...
 *
 */
public class R_S004_B002 implements LinkeyRule {
    @Override
    ...
    public void uploadFile() {
        try {
            ...
            BeanCtx.getResponse().sendRedirect("rule?wf_num=R_S004_B002&appId=" + appId + "&FdName=" + doc.g("FdName") + "&reload=1"); //加入重新显示附件列表标记位
        }
        catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void showUploadForm() {
        ...
        htmlBf.append("    <style>");
        htmlBf.append("    *{margin:0px; padding:0px;}"); //使内外边距都为0，完全显示附件上传
        htmlBf.append("      #uploadImg { font-size:12px;  position:absolute;}");
        htmlBf.append("      #selectfile{cursor:pointer;position:absolute;z-index:100;left:-23px;opacity:0;filter:alpha(opacity=0);top:5px;width:100px; }");
        htmlBf.append("      #uploadtext{background-image:url(" + BeanCtx.getMsg("Common", "UploadButtonText") + ");background-repeat:no-repeat;display:block;float:left;height:20px;margin-top:0px;position:relative;text-decoration:none;top:0pt; width:80px;cursor:pointer;}");
        htmlBf.append("    </style>");
        ...
        htmlBf.append("    <form method=\"post\" action=\"rule?wf_num=R_S004_B002&appId=" + BeanCtx.g("appId", true) + "\" enctype=\"multipart/form-data\" accept-charset=\"utf-8\" name=\"linkeyform\">");
        ...
        htmlBf.append("        <a href=\"#\" id=\"uploadtext\"  hidefocus=\"true\"></a>");
        htmlBf.append("      </span>");
