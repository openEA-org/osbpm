 ...
 */
final public class R_S026_E002 implements LinkeyRule {
    ...
    public String onFormOpen(Document doc, Document formDoc, String readOnly) throws Exception {
        ...
        if (!nodeDoc.isNull()) {
            //使用设置的用户
            doc.s("NodeOwner", nodeDoc.g("NodeOwner"));
        }
        else {
            //使用节点中配置的用户
            ModNode modNode = (ModNode) BeanCtx.getBean("ModNode");
            nodeDoc = modNode.getNodeDoc(processid, nodeid);
            if (nodeDoc.g("UsePostOwner").equals("1")) {
                doc.s("NodeOwner", nodeDoc.g("ApprovalFormOwner"));
            }
            else {
                doc.s("NodeOwner", nodeDoc.g("PotentialOwner"));
            }
        }
        doc.s("NodeOwner_show", BeanCtx.getLinkeyUser().getCnName(doc.g("NodeOwner")));
