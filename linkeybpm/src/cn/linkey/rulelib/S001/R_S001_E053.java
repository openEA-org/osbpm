 ...
 */
final public class R_S001_E053 implements LinkeyRule {
    ...
    public String onFormOpen(Document doc, Document formDoc, String readOnly) {
        ...
        if (Rdb.getDbType().equals("MSSQL")) {
            sql = "select AppName+'('+WF_Appid+')',WF_Appid from BPM_AppList where " + Rdb.getInReaderSql("Owner");
        }
        else if (Rdb.getDbType().equals("MYSQL")) {
            sql = "select concat(AppName,'(',WF_Appid,')'),WF_Appid from BPM_AppList where " + Rdb.getInReaderSql("Owner");
        }
        else {
            sql = "select AppName||'('||WF_Appid||')',WF_Appid from BPM_AppList where " + Rdb.getInReaderSql("Owner");
        }
