 ...
 */
public class R_S001_E008 implements LinkeyRule {
	...
	public String onFormOpen(Document doc, Document formDoc, String readOnly) {
		...
		if (doc.isNewDoc()) {
			...
			String sql = "select FormNumber from BPM_FormList where WF_Appid='" + appid
					+ "' and FormType='1' order by FormNumber desc";
			String newNum = AppUtil.getElNewNum(sql);
			if (Tools.isBlank(newNum)) {
				newNum = "F_" + appid + "_A001";
			}
