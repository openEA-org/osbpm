 ...
 * 
 */
public class R_S001_E007 implements LinkeyRule {
    ...
    public String onFormOpen(Document doc, Document formDoc, String readOnly) {
        ...
        String defaultValue = doc.g("ParentFolderid");
        if (doc.isNewDoc()) {
            defaultValue = "root";
        }
        ...
        if (Rdb.getDbType().equals("MSSQL")) {
            sql = "select AppName+'('+WF_Appid+')',WF_Appid from BPM_AppList where IsFolder='1'";
        }
        else if (Rdb.getDbType().equals("MYSQL")) {
            sql = "select concat(AppName,'(',WF_Appid,')'),WF_Appid from BPM_AppList where IsFolder='1'";
        }
        else {
            sql = "select AppName||'('||WF_Appid||')',WF_Appid from BPM_AppList where IsFolder='1'";
        }
        ...
        doc.s("ParentFolderid", applist);

        //应用模板列表
        if (doc.isNewDoc()) {
            if (Rdb.getDbType().equals("MSSQL")) {
                sql = "select AppName+'('+WF_Appid+')',WF_Appid from BPM_AppList where IsTemplate='1'";
            }
            else if (Rdb.getDbType().equals("MYSQL")) {
                sql = "select concat(AppName,'(',WF_Appid,')'),WF_Appid from BPM_AppList where IsTemplate='1'";
            }
            else {
                sql = "select AppName||'('||WF_Appid||')',WF_Appid from BPM_AppList where IsTemplate='1'";
            }
            applist = Rdb.getValueForSelectTagBySql(sql, defaultValue);
            applist = "-无-|root," + applist;
            doc.s("WF_TempApp", applist);
        }
        else {
            doc.s("WF_TempApp", "-无-");
        }
    ...
    public String onFormSave(Document doc, Document formDoc) throws Exception {
        ...
        if (Tools.isBlank(doc.g("IsFolder"))) {
            doc.s("IsFolder", "0");
        }

        //看是否需要复制模板
        String srcAppid = doc.g("WF_TempApp");
        if (Tools.isNotBlank(srcAppid) && !srcAppid.equals("root")) {
            String targetAppid = appid;
            int i = AppUtil.copyApp(srcAppid, targetAppid, srcAppid, targetAppid);
        }
