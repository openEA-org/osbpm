 ...
 * 
 */
public class R_S001_E004 implements LinkeyRule {
    ...
    public String onFormOpen(Document doc, Document formDoc, String readOnly) {
        ...
        if (doc.isNewDoc()) {
            ...
            String sql = "select RuleNum from BPM_RuleList where WF_Appid='" + appid + "' and RuleType='2' order by RuleNum desc";
            String newRuleNum = AppUtil.getElNewNum(sql);
            if (Tools.isBlank(newRuleNum)) {
                newRuleNum = "R_" + appid + "_B001";
            }
    ...
    public String onFormSave(Document doc, Document formDoc) {
        ...
        String compileFlag = doc.g("CompileFlag");
        if (Tools.isBlank(compileFlag)) {
            compileFlag = "0";
        }
        doc.s("CompileFlag", compileFlag);
        doc.s("Singleton", "1");
        doc.s("Status", "1");
        doc.s("WF_CatchFlag", "0");
        doc.s("EventType", "0");
