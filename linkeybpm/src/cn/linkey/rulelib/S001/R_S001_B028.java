 ...
 */
final public class R_S001_B028 implements LinkeyRule {
    @Override
    ...
    public String newFolder() {
        ...
        String appid = Rdb.getValueBySql("select WF_Appid from BPM_NavTreeList where Treeid='" + treeid + "'");
        ...
        if (parentDocUnid.equals("root")) {
            ...
            String sql = "select Folderid from BPM_NavTreeNode where WF_Appid='" + appid + "' and Treeid='" + treeid + "' and ParentFolderid='root'";
            ...
            if (allSubFolderSet.size() == 0) {
                ...
                sql = "select SortNum from BPM_NavTreeNode where WF_Appid='" + appid + "' and Treeid='" + treeid + "' and ParentFolderid='root' order by SortNum desc";
            ...
            //在其他文件夹下面创建
            String sql = "select * from BPM_NavTreeNode where WF_OrUnid='" + parentDocUnid + "'";
            Document parentDoc = Rdb.getDocumentBySql(sql);
            parentFolderid = parentDoc.g("Folderid");

            //获得上级文件夹下面的所有子文件夹的已有编号
            sql = "select Folderid from BPM_NavTreeNode where WF_Appid='" + appid + "' and Treeid='" + treeid + "' and ParentFolderid='" + parentFolderid + "'";
            HashSet<String> allSubFolderSet = Rdb.getValueSetBySql(sql);
            //BeanCtx.out(allSubFolderSet.toString());
            if (allSubFolderSet.size() == 0) {
                //还没有子文件夹的情况下
                ...
                //计算最大的排序号,让新增的文件夹位于排序的最后
                sql = "select SortNum from BPM_NavTreeNode where WF_Appid='" + appid + "' and Treeid='" + treeid + "' and ParentFolderid='" + parentFolderid + "' order by SortNum desc";
                newSortNum = Rdb.getValueTopOneBySql(sql);
                newSortNum = String.valueOf(Integer.parseInt(newSortNum) + 1);
            }
        ...
        //创建一个子菜单文档对像
        Document newSubDoc = BeanCtx.getDocumentBean("BPM_NavTreeNode");
        newSubDoc.s("WF_OrUnid", Rdb.getNewid("BPM_NavTreeNode"));
        newSubDoc.s("FolderName", folderName);
        newSubDoc.s("WF_Appid", appid);
        ...
        if (i > 0) {
            ...
            BeanCtx.p(Tools.jmsg("error", BeanCtx.getMsg("Designer", "NewNavTreeItemError")));
        }
    ...
    public void editFolder() {
        ...
        String sql = "select * from BPM_NavTreeNode where WF_OrUnid='" + docUnid + "'";
    ...
    public void moveFolder() {
        ...
        String treeid = BeanCtx.g("treeid", true);
        //	    String appid=BeanCtx.g("WF_Appid",true); //所属应用
        String newSortNum = "1001"; //新的排序号
        String newSubFolderid = ""; //新的文件夹编号
        String parentFolderid = "root"; //上级文件夹编号,默认为root
        if (parentDocUnid.equals("root")) {
            //移动到根文件夹下面
            //获得root文件夹下面的所有子文件夹的已有编号
            String sql = "select Folderid from BPM_NavTreeNode where Treeid='" + treeid + "' and ParentFolderid='root'";
            HashSet<String> allSubFolderSet = Rdb.getValueSetBySql(sql);
            // BeanCtx.out(allSubFolderSet.toString());
            if (allSubFolderSet.size() == 0) {
                //还没有子文件夹的情况下
                ...
                //计算最大的排序号,让新增的文件夹位于排序的最后
                sql = "select SortNum from BPM_NavTreeNode where  Treeid='" + treeid + "' and ParentFolderid='root' order by SortNum desc";
                newSortNum = Rdb.getValueTopOneBySql(sql);
                newSortNum = String.valueOf(Integer.parseInt(newSortNum) + 1);
            }
        }
            ...
            //获得上级文件夹下面的所有子文件夹的已有编号
            sql = "select Folderid from BPM_NavTreeNode where Treeid='" + treeid + "' and ParentFolderid='" + parentFolderid + "'";
            HashSet<String> allSubFolderSet = Rdb.getValueSetBySql(sql);
            // BeanCtx.out(allSubFolderSet.toString());
            if (allSubFolderSet.size() == 0) {
                //还没有子文件夹的情况下
                ...
                //计算最大的排序号,让新增的文件夹位于排序的最后
                sql = "select SortNum from BPM_NavTreeNode where Treeid='" + treeid + "' and ParentFolderid='" + parentFolderid + "' order by SortNum desc";
                newSortNum = Rdb.getValueTopOneBySql(sql);
                newSortNum = String.valueOf(Integer.parseInt(newSortNum) + 1);
            }
        ...
        String sql = "select * from BPM_NavTreeNode where WF_OrUnid='" + curDocUnid + "'";
        Document doc = Rdb.getDocumentBySql(sql);
    ...
    public void moverAllSubFolder(String treeid, String oldFolderid, String newFolderid) {
        String sql = "select * from BPM_NavTreeNode where Treeid='" + treeid + "' and ParentFolderid like '" + oldFolderid + "%'";
