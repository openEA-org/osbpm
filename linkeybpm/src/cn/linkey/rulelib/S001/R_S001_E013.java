...
/**
 * @RuleName:新增Json数据源表单事件
 * @author admin
 * @version:
 * @Created: 2014-03-30 23:09
 */
public class R_S001_E013 implements LinkeyRule {
    ...
    public String onFormOpen(Document doc, Document formDoc, String readOnly) {
        ...
        if (doc.isNewDoc()) {
            ...
            String sql = "select Dataid from BPM_DataSourceList where WF_Appid='" + appid + "' and DataType='1' order by Dataid desc";
            //BeanCtx.out("sql="+sql);
            String newNum = AppUtil.getElNewNum(sql);
            if (Tools.isBlank(newNum)) {
                newNum = "D_" + appid + "_J001";
            }
            doc.s("Dataid", newNum);
        }
