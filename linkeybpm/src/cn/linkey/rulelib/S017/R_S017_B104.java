...
/**
 * @RuleName:Rest_Engine回退首环节
 * @author  admin
 * @version: 1.0
 */
final public class R_S017_B104 implements LinkeyRule {
	@Override
	public String run(HashMap<String, Object> params) throws Exception  {
        ...
        String isBackFlag=BeanCtx.g("isBackFlag"); //回退类型空或者1表示回退后需要重新提交审批,2表示直接返回给回退者
        ...
        runParams.put("WF_IsBackFlag", isBackFlag); //指定重新提交后的类型
        runParams.put("WF_Remark", remark) ;//办理意见
        
//        BeanCtx.out("提交运行参数runParams="+runParams);
        
        //6.提交工作流引擎运行
        String msg = linkeywf.run("GoToFirstNode", runParams);
        if (linkeywf.isDebug()) {
