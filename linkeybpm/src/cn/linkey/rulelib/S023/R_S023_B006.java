 ...
 */
final public class R_S023_B006 implements LinkeyRule {
    @Override
    public String run(HashMap<String, Object> params) throws Exception {
        //params为运行本规则时所传入的参数
        String tableName = "BPM_ArchivedData";
        ...
        String listCode = "<li onclick=\"window.open('r?wf_num=R_S003_B062&wf_docunid={WF_OrUnid}&mobile=1','_self')\"><h2>{NO}.{Subject}</h2><span style=\"color:#666;font-size:9pt\"><p>流程名称:{WF_ProcessName}  申请人:{WF_AddName_CN}<br>总耗时:{TotalTime}</p></span></li>";

        int no = (pageNum - 1) * Integer.parseInt(limit);
        ;
        String sql = "select WF_DocNumber,Subject,WF_Author_CN,WF_AddName_CN,WF_DocCreated,WF_ProcessName,WF_CurrentNodeName,WF_OrUnid from " + tableName + " where ','+WF_AllReaders+',' like '%,"
                + BeanCtx.getUserid() + ",%'  order by WF_DocCreated DESC";
