...
/**
 * 本类负责生成category grid视图
 * 
 * @author Administrator 本类为单例类
 */
public class AppCategoryGrid implements AppElement {
    ...
    public String getElementBody(String gridNum, boolean readOnly) throws Exception {
        ...
        if (Tools.isBlank(jsHeader)) {
            jsHeader = Rdb.getValueBySql("select DefaultCode from BPM_DevDefaultCode where CodeType='CategoryGrid_JsHeader'");
        }
        else {
    ...
    public String getGridColumn(Document gridDoc) {
        ...
        dataoptions.append(",idField:'id',treeField:'text'"); // 增加树的两个标识字段
    ...
    public String getToolbar(Document gridDoc) {
        ...
        if (Tools.isBlank(toolbarJson)) {
            toolbarJson = Rdb.getValueBySql("select DefaultCode from BPM_DevDefaultCode where CodeType='CategoryGrid_ToolBar'");
        }
