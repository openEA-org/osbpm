...
* @Description: 本类主要负打开应用页面并输出html (layui处理)
*
* @version: v1.0.0
* @author: alibao
* @date: 2018年3月9日 下午5:19:02 
*
    ...
    public String getElementHtml(String pageNum) throws Exception {
        ...
        if (!noPageHeader.equals("1")) { //不输出html头
            ...
            if (Tools.isBlank(htmlHeaderConfig)) {
                htmlHeaderConfig = "layui_AppPageHtmlHeader";
            }
        ...
        if (Tools.isBlank(jsHeader)) {
            if (!noPageHeader.equals("1")) {
                jsHeader = Rdb.getValueBySql("Select DefaultCode from BPM_DevDefaultCode where CodeType='layui_Page_JsHeader'");
            }
        }
        else {
        ...
        if (this.isValidSysDate()) {
            return "Error:this system sn has expired!";
        }
