...
public class ProcessEngine {
    ...
    public void endParentSubProcessNode(HashMap<String, Object> runParams) throws Exception {
        ...
        if (Tools.isNotBlank(mainDocUnid) && !mainDocUnid.equals(getDocUnid())) {
            ...
            if (endModNodeDoc.g("BackToMainProcess").equals("1")) {
                //说明结束环节中要求返回主流程
                Document parentDocument = Rdb.getDocumentBySql("select * from " + this.mainTableName + " where WF_OrUnid='" + mainDocUnid + "'");
                String mainProcessid = parentDocument.g("WF_Processid"); //获得主流程的processid
                if (BeanCtx.getLinkeywf().isDebug()) {
                    BeanCtx.out("准备返回主流程Processid=" + mainProcessid + ",主流程节点=" + mainNodeid + ",主流程实例docunid=" + mainDocUnid);
                }
    ...
    public String open() throws Exception {
        ...
        // 查询流程模型Document
        Document processDoc = Rdb.getDocumentBySql("select * from BPM_ModProcessList where Processid = '" + processid + "'");
        if (isFirstNode) {
            // 获取流程有权启动者
        ...
        if (Tools.isNotBlank(this.currentNodeid) && !isNewProcess) {
            if (!this.currentInsUserDoc.g("WF_IsNewInsUserDoc").equals("1") && Tools.isBlank(this.currentInsUserDoc.g("FirstReadTime"))) {
        ...
        if (Tools.isNotBlank(copyUserList)) {
            ...
            if (copyUserList.indexOf("," + BeanCtx.getUserid() + ",") != -1) {
                ...
                if (document.g("WF_Status").equals("ARC")) {
                    ...
                    //已归档的文件则需要清除待阅记录
                    if (Tools.isNotBlank(copyUserList)) {
                        Rdb.execSql("delete from BPM_InsCopyUserList where Userid='" + BeanCtx.getUserid() + "' and DocUnid='" + docUnid + "'");
                    }
                    else {
        ...
        HashMap<String, LinkedHashSet<String>> engineFormCache = (HashMap<String, LinkedHashSet<String>>) RdbCache.getSystemCache("BPM_EngineFormPluginConfig", "ALL"); //含PC和Mobile全部
     ...
     */
    public void runNode(String processid, String nodeid, String ruleType, HashMap<String, Object> runParams) throws Exception {
        ModNode insModNode = ((ModNode) BeanCtx.getBean("ModNode"));
        Document nodeDoc = insModNode.getNodeDoc(processid, nodeid);
        runParams.put("WF_RunNodeid", nodeid); //默认加入运行节点的参数
        String ruleNum = ((RuleConfig) BeanCtx.getBean("RuleConfig")).getNodeTypeConfig(nodeDoc.g("ExtNodeType"), ruleType);
        if (Tools.isBlank(ruleNum)) {
            BeanCtx.log("E", "不支持的节点类型(" + nodeDoc.g("ExtNodeType") + "),请在BPM_RuleNodeTypeConfig中配置相应的节点和" + ruleType + "规则!");
        }
    ...
    public String getRunMsg() {
        ...
        if (this.getEndNodeid() == null) {
            ...
            String sql = "select NodeName from BPM_InsNodeList where DocUnid='" + docUnid + "' and (NodeType='Task' or NodeType='SubProcess') and ActionNum='" + actionNum + "' and Status='Current'";
    ...
    public int canSelectNodeAndUser() {
        ...
        if (currentModNodeDoc.g("IsSequential").equals("1")) {
            ...
            String sql = "select * from BPM_InsUserList where DocUnid='" + docUnid + "' and Nodeid='" + currentNodeid + "' and Status='Wait' order by SerialIndexNum";
            if (Rdb.hasRecord(sql)) {
                return 1;
            }
        }
        ...
        if (currentModNodeDoc.g("LoopType").equals("3")) {
            ...
            String sql = "select WF_OrUnid from BPM_InsUserList where docUnid='" + docUnid + "' and Userid<>'" + BeanCtx.getUserid() + "' and nodeid='" + currentNodeid + "' and Status='Current'";
            if (Rdb.hasRecord(sql)) {
    ...
    public void saveStayData(String processid, String nodeid, String docUnid, HashMap<String, Object> params) {
        ...
        HashMap<String, HashMap<String, String>> nextUserDept = (HashMap<String, HashMap<String, String>>) params.get("WF_NextUserDept");//兼职时有用 key为nodeid,value为用户与部门id的map组合
        ...
        for (String nextNodeid : nextNodeList) {
            ...
            //存临时的驻留数据
            Document doc = Rdb.getDocumentBySql("BPM_InsStayData",
                    "select * from BPM_InsStayData where DocUnid='" + docUnid + "' and Userid='" + BeanCtx.getUserid() + "' and WF_NextNodeid='" + nextNodeid + "'");
